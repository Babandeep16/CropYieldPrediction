...
To keep predictions realistic, we:
- limit inputs to the **5th–95th percentile** of historical data, and  
- apply a simple **agronomic sanity correction** when climate is outside
  typical ranges for a crop.
"""
)
...
HIST_MAX_YEAR = int(df["Year"].max())  # 2013
...
# --------------------------------------------------
# Simple crop-specific climate rules (very rough, for sanity only)
# values are approximate ranges where the crop usually performs well
# --------------------------------------------------
CROP_RULES = {
    ...
    "Plantains and others": {"min_rain": 700, "min_temp": 20, "max_temp": 32},
}

def apply_sanity_correction(crop, rainfall, temp, raw_pred):
    """
    Simple penalty based on very rough climate suitability.
    Returns (corrected_pred, explanation_str)
    """
    rules = CROP_RULES.get(crop)
    if rules is None:
        return raw_pred, "No crop-specific correction applied."
    ...
    if rainfall < rules["min_rain"]:
        ...
        penalty *= factor
        messages.append("rainfall below typical range")
    # we don't penalize very high rainfall (some crops tolerate it)

    # Temperature penalty
    if temp < rules["min_temp"]:
        factor = max(temp / rules["min_temp"], 0.1)
        penalty *= factor
        messages.append("temperature below typical range")
    if temp > rules["max_temp"]:
        # above optimum → yield drops
        factor = max((rules["max_temp"] / temp), 0.1)
        penalty *= factor
        messages.append("temperature above typical range")

    corrected = raw_pred * penalty

    if messages:
        msg = (
            "Sanity correction applied due to: "
            + ", ".join(messages)
            + ". Adjusted yield downwards."
        )
    else:
        msg = "Climate conditions within typical range for this crop."

    return corrected, msg

# --------------------------------------------------
# Helper: flag extremes within historical range
# --------------------------------------------------
def flag_extremes(rainfall, temp, pesticides, container):
    ...
    if messages:
        container.warning(
            ...
            + ", ".join(messages)
            + "). Model predictions here may be less reliable."
        )
...
if page == "Single Prediction":
    ...
    if st.button("Predict Yield") and model is not None:
        ...
        corrected_pred, explanation = apply_sanity_correction(
            item, rainfall, temp, raw_pred
        )

        st.success(f"### 🌱 Model Prediction (Raw): **{raw_pred:,.0f} hg/ha**")
        st.success(f"### 🌱 Sanity-Adjusted Yield: **{corrected_pred:,.0f} hg/ha**")
        st.caption(
            "Model: Random Forest Regressor + preprocessing. "
            + explanation
        )
...
# --------------------------------------------------
    ...
    if model is not None:
        ...
        raw_pred = model.predict(input_df)[0]
        corrected_pred, explanation = apply_sanity_correction(
            base_item, rainfall, temp, raw_pred
        )

        st.metric("Raw Model Yield (hg/ha)", f"{raw_pred:,.0f}")
        st.metric("Sanity-Adjusted Yield (hg/ha)", f"{corrected_pred:,.0f}")
        st.caption(
            "Agronomic sanity check: " + explanation
        )
...
# --------------------------------------------------
    ...
    if model is not None:
        ...
        **Important:** Model accuracy is measured on historical data; the
        agronomic sanity layer helps keep individual predictions realistic
        when users test extreme climate scenarios.
        """
        )
